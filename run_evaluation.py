...
def run_stage(stage: str, client: LLMClient, lang: str = "en") -> dict:
    ...
    if stage == "bbq":
        ...
        for r in weat_results:
            ...
            p = r.get("p_value")
            d_str = f"{float(d):.3f}" if isinstance(d, (int, float)) else str(d)
            p_str = f"{float(p):.3f}" if isinstance(p, (int, float)) else str(p)
            print(f"  {r['test']:25s} ({r['language']})  d={d_str}  p={p_str}")
        ...
        print("ABM empty:", abm_df.empty)
        print("ABM shape:", abm_df.shape)
        print("ABM columns:", list(abm_df.columns))

        if not abm_df.empty:
            abm_df.to_csv(
                RESULTS_DIR / f"abm_results_{client.model_key}.csv",
                index=False
            )
            ...
            for _, row in abm_df.iterrows():
                ...
                print(
                    f"  {row['scenario']:20s} ({row['language']})  "
        ...
        if not abm_events_df.empty:
            abm_events_df.to_csv(
                RESULTS_DIR / f"abm_events_{client.model_key}.csv",
                index=False
            )
    return result
